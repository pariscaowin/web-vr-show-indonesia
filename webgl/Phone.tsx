...
export default function Phone(props:any) {
  ...
  const gltf:any = useLoader(GLTFLoader, '/assets/models/phone/phone.glb', (loader:any) => {
  ...
  return (
    <>
      <mesh ref = {phoneRef} visible={props.visible}>
        <primitive object={gltf.scene} position={props.position} scale={props.scale} rotation={[0.0, -Math.PI/4, 0]}>          
          <Html position={[0, 0.18, 0]} style={{display:showPhone?'block':'none'}}>
            <div style={{transform: 'translate3d(-50%, -117%, 0px)'}}>              
                ...
                >
                  ...
                  {/* <div style={{margin:'8px 12px 8px 0px', fontSize:'30px', lineHeight:'30px'}}>
                    Phone
                  </div> */}
                </div>              
            </div>
